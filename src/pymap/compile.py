...
def get_metadata(datatype_str: str, label: str, project: Project) -> str:
    """Creates the metadata for a datatype.

    Args:
        datatype_str (str): The name of the datatype.
        label (str): The label of the datatype.
        project (Project): The pymap project.

    Returns:
        str: The metadata string for the datatype.
    """
    return (
        f'@ Data for {label} of type {datatype_str} in project {project.path}\n'
        f'@ Generated by at {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\n'
    )
...
def datatype_to_assembly(
    ...
    assembly, additional_blocks = datatype.to_assembly(
        data, project, [], [], label=label, alignment=2, global_label=True
    )
    blocks = (
        [
