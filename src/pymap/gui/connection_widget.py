...
class ConnectionWidget(QWidget):
    ...
    def open_adjacent_map(self):
        ...
        if (
            ...
            return
        packed = properties.get_member_by_path(
            self.main_gui.header,
            self.main_gui.project.config['pymap']['header']['connections'][
                'connections_path'
            ],
        )[self.idx_combobox.currentIndex()]
        # Update the unpacked version
        direction, offset, bank, map_idx, connection_blocks = blocks.unpack_connection(
            packed, self.main_gui.project
        )
        try:
            ...
            return QMessageBox.critical(
                ...
                f'The header {bank}.{map_idx} could not be opened. Key {e.args[0]} was not found.',
            )
        ...
        if mirror_offset and self.connections[connection_idx] is not None:
            ...
            if header is not None:
                ...
                for idx, adjacent_connection in enumerate(adjacent_connections):
                    ...
                    try:
                        adjacent_map_idx = str(int(str(adjacent_map_idx), 0))
                    except ValueError:
                        pass
                    try:
                        adjacent_bank = str(int(str(adjacent_bank), 0))
                    except ValueError:
                        pass
                    if (
                        opposite_directions[connection_type] == adjacent_connection_type
    ...
    def update_blocks(self):
        ...
        for y, x in zip(*np.where(new_blocks[:, :, 0] != self.blocks[:, :, 0])):
            pixmap = QPixmap.fromImage(
                ImageQt(self.main_gui.blocks[new_blocks[y, x, 0]])
            )
    ...
    def update_border_rectangles(self):
        ...
        for idx, connection in enumerate(
            ...
            self.connection_rects[direction].setRect(
                *fix_rect(
                    *rect,
                    16 * (map_width + 2 * padded_width),
                    16 * (map_height + 2 * padded_height),
            ...
            if idx == self.idx_combobox.currentIndex():
                self.connection_rects[direction].setPen(
                    QPen(connection_active_border_color)
                )
                self.connection_rects[direction].setBrush(
...
class MapScene(QGraphicsScene):
    ...
    def mouseMoveEvent(self, event):
        ...
        if (
            ...
            return
        map_width = properties.get_member_by_path(
            self.connection_widget.main_gui.footer,
            self.connection_widget.main_gui.project.config['pymap']['footer'][
                'map_width_path'
            ],
        )
        map_height = properties.get_member_by_path(
            ...
            for direction in self.connection_widget.connection_rects:
                ...
                if (
                    ...
                    if self.dragged_idx == -1:
                        raise RuntimeError(
                            f'Inconsistent connection type {direction}. Did not find any matching connection for a rectangle.'
                        )
