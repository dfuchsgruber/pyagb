...
class EventTab(QWidget):
    ...
    def __init__(self, event_widget, event_type, parent=None):
        ...
        self.remove_button.clicked.connect(
            lambda: self.remove_event(self.idx_combobox.currentIndex())
        )
        ...
        if event_type.get('goto_header_button_button_enabled', False):
            ...
            self.goto_header_button.clicked.connect(
                lambda: self.goto_header(self.idx_combobox.currentIndex())
            )
    ...
    def remove_event(self, event_idx):
        ...
        self.event_widget.undo_stack.push(
            history.RemoveEvent(self.event_widget, self.event_type, event_idx)
        )
    ...
    def append_event(self):
        ...
        if (
            ...
            return
        self.event_widget.undo_stack.push(
            history.AppendEvent(self.event_widget, self.event_type)
        )
    ...
    def goto_header(self, event_idx: int):
        ...
        try:
            ...
            return QMessageBox.critical(
                ...
                f'The header {target_bank}.{target_map_idx} could not be opened. Key {e.args[0]} was not found.',
            )
...
class MapScene(QGraphicsScene):
    ...
    def update_selection(self):
        ...
        padded_x, padded_y = self.event_widget.main_gui.map_widget.get_border_padding()
        x, y = to_coordinates(
            properties.get_member_by_path(events[idx], tab.event_type['x_path']),
            properties.get_member_by_path(events[idx], tab.event_type['y_path']),
            padded_x,
            padded_y,
        )
    ...
    def mouseMoveEvent(self, event):
        ...
        map_width = properties.get_member_by_path(
            self.event_widget.main_gui.footer,
            self.event_widget.main_gui.project.config['pymap']['footer'][
                'map_width_path'
            ],
        )
    ...
    def mousePressEvent(self, event):
        ...
        if target is not None:
            ...
            self.event_widget.tab_widget.setCurrentWidget(
                self.event_widget.tabs[event_type['datatype']]
            )
            self.event_widget.tabs[event_type['datatype']].idx_combobox.setCurrentIndex(
                event_idx
            )
...
class EventGroupImage(QGraphicsItemGroup):
    ...
    def __init__(
        self, map_scene, image, horizontal_displacement, vertical_displacement
    ):
        super().__init__()
