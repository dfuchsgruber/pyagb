...
class MapWidget(QWidget):
    ...
    def __init__(self, main_gui, parent=None):
        ...
        self.level_opacity_slider = QtWidgets.QCheckBox(Qt.Horizontal)
        ...
        self.level_blocks_pixmap = QPixmap(
            os.path.join(os.path.split(__file__)[0], 'level_blocks.png')
        )
        # And split them
        ...
        self.combo_box_tileset_primary.currentTextChanged.connect(
            lambda label: self.main_gui.change_tileset(label, True)
        )
        self.combo_box_tileset_secondary.currentTextChanged.connect(
            lambda label: self.main_gui.change_tileset(label, False)
        )
        group_tileset.setLayout(tileset_layout)
        tileset_layout.setColumnStretch(0, 0)
        tileset_layout.setColumnStretch(1, 1)

        # group_shift = QtWidgets.QGroupBox('Shift')
        # blocks_container.addWidget(group_shift)
        # shift_layout = QtWidgets.QGridLayout()
        # self.shift_left = QtWidgets.QPushButton('←')
        # shift_layout.addWidget(self.shift_left, 1, 1, 2, 1)
        # self.shift_up = QtWidgets.QPushButton('↑')
        # shift_layout.addWidget(self.shift_up, 1, 2, 1, 1)
        # self.shift_down = QtWidgets.QPushButton('↓')
        # shift_layout.addWidget(self.shift_down, 2, 2, 1, 1)
        # self.shift_right = QtWidgets.QPushButton('→')
        # shift_layout.addWidget(self.shift_right, 1, 3, 2, 1)
        # group_shift.setLayout(shift_layout)
    ...
    def resize_map(self):
        ...
        input, ok_pressed = QInputDialog.getText(
            ...
            f'Enter new dimensions of footer {self.main_gui.footer_label} in the format "width,height".',
            text=f'{width},{height}',
        )
        if not ok_pressed:
            return
        ...
        if width_new > width_max or width_new <= 0:
            return QMessageBox.critical(
                self,
                'Invalid Dimeinsions',
                f'Width {width_new} larger than maximum width {width_max} or non positive',
            )
        try:
        ...
        if height_new > height_max or height_new <= 0:
            return QMessageBox.critical(
                self,
                'Invalid Dimeinsions',
                f'Height {height_new} larger than maximum height {height_max} or non positive',
            )
    ...
    def resize_border(self):
        ...
        input, ok_pressed = QInputDialog.getText(
            ...
            f'Enter new dimensions of border of footer {self.main_gui.footer_label} in the format "width,height".',
            text=f'{width},{height}',
        )
        if not ok_pressed:
            return
    ...
    def load_border(self):
        ...
        self.border_image = QPixmap.fromImage(
            ImageQt(render.draw_block_map(self.main_gui.blocks, border_blocks))
        )
    ...
    def load_auto_shapes(self):
        ...
        self.auto_shapes_scene._update_pixmap()
    ...
    def set_blocks_selection(self, selection):
        ...
        selection_pixmap = QPixmap.fromImage(
            ImageQt(render.draw_block_map(self.main_gui.blocks, self.selection))
        )
...
class BorderScene(QGraphicsScene):
    ...
    def mouseMoveEvent(self, event):
        ...
        borders = properties.get_member_by_path(
            ...
            self.map_widget.main_gui.project.config['pymap']['footer']['border_path'],
        )
        pos = event.scenePos()
        ...
        if self.selection_box is not None:
            ...
            if x1 != x + 1 or y1 != y + 1:
                ...
                self.map_widget.set_selection(
                    render.select_blocks(render.block_map, *self.selection_box)
                )
...
class MapScene(QGraphicsScene):
    ...
    def mouseMoveEvent(self, event):
        ...
        if x in range(border_width, border_width + map_width) and y in range(
            border_height, border_height + map_height
        ):
            if self.map_widget.tabs.currentIndex() == 1:
                ...
                self.map_widget.info_label.setText(
                    f'x : {hex(x - border_width)}, y : {hex(y - border_height)}, Block : {hex(block[0])}, Level : {hex(block[1])}'
                )
            ...
            if not self.last_draw is None and self.last_draw != (x, y):
                ...
                # print(f'Is smart drawing?', self.smart_drawing)
                if self.smart_drawing is not None:
                    # print(f'In smart drawing.')
                    if (
                        QApplication.keyboardModifiers() != Qt.AltModifier
                    ):  # Force-end the smart drawing
                        self.smart_drawing = None
                        ...
                        for i in range(
                            10000
                        ):  # Limiting the number of iterations is bug-saftey measure, this loop should always terminate before...
                        ...
                        for coordinate in queue:
                            ...
                            for idx in (-2, -1, 0):
                                (y, x), shape_idx = self.smart_drawing.get_by_path_idx(
                                    idx
                                )
...
class AutoScene(QGraphicsScene):
    ...
    def _update_pixmap(self):
        ...
        self.auto_shape_background_pixmap = QPixmap(
            os.path.join(os.path.split(__file__)[0], 'auto_shape_background.png')
        )
        ...
        self.blocks_image = QPixmap.fromImage(
            ImageQt(
                render.draw_block_map(self.map_widget.main_gui.blocks, self.auto_shape)
            )
        )
        item = QGraphicsPixmapItem(self.blocks_image)
        self.addItem(item)
