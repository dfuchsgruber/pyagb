...
def get_border(footer: ModelValue, block_images: BlockImages,
    ...
    """
    width, height = (get_member_by_path(footer,
                                        project.config['pymap']['footer'][dimension])
    ...
    border_blocks = get_member_by_path(footer,
                                       project.config['pymap']['footer']['border_path'])
    ...
    for y, block_line in enumerate(border_blocks):
        ...
        for x, block_data in enumerate(block_line):
            ...
            border_img.paste(block_images[block_idx], (16 * x, 16 * y),
                             block_images[block_idx])
    return border_img
...
def draw_blocks(block_images: BlockImages, blocks_pool: npt.NDArray[np.int_]):
    ...
    for (y, x), block_idx in np.ndenumerate(blocks_pool[:, :, 0]):
        map_img.paste(block_images[block_idx], (16 * x, 16 * y),
                      block_images[block_idx])
    return map_img

def get_blocks(tileset_primary: ModelValue, tileset_secondary: ModelValue,
               tiles: TileImages, project: Project) -> list[Image.Image]:
    ...
    blocks : list
        A list of 16x16 block images.
    """
    blocks_primary = get_member_by_path(tileset_primary,
                                        project.config['pymap']['tileset_primary']['blocks_path'])
    blocks_secondary = get_member_by_path(tileset_secondary,
                                          project.config['pymap']['tileset_secondary']['blocks_path'])
    assert isinstance(blocks_primary, list)
    assert isinstance(blocks_secondary, list)
    return [get_block(block, tiles)
            for block in blocks_primary + blocks_secondary]
...
def get_tiles(tileset_primary: ModelValue, tileset_secondary: ModelValue,
              project: Project) -> list[list[Image.Image]]:
    ...
    """
    # Load images
    gfx_primary = get_member_by_path(tileset_primary,
                                     project.config['pymap']['tileset_primary']['gfx_path'])
    gfx_secondary = get_member_by_path(tileset_secondary,
                                       project.config['pymap']['tileset_secondary']['gfx_path'])
    ...
    palettes_primary = get_member_by_path(tileset_primary,
                                          project.config['pymap']['tileset_primary']['palettes_path'])
    palettes_secondary = get_member_by_path(tileset_secondary,
                                            project.config['pymap']['tileset_secondary']['palettes_path'])

    palettes = pack_colors(palettes_primary) + pack_colors(palettes_secondary)
    return [
            [
                img for row in split_image_into_tiles(
                    gfx_primary.to_pil_image(palette, transparent=0)) for img in row
            ] + \
            [
                img for row in split_image_into_tiles(
                    gfx_secondary.to_pil_image(palette, transparent=0)) for img in row
            ]
...
def select_blocks(blocks: npt.NDArray[np.int_], x0: int,
                  x1: int, y0: int, y1: int) -> npt.NDArray[np.int_]:
