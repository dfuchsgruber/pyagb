...
class BlockScene(QGraphicsScene, TilesetChildWidgetMixin):
    ...
    def __init__(self, tileset_widget: TilesetWidget, layer: int,
    ...
    def update_block(self):
        ...
        for (y, x), tile in np.ndenumerate(block.reshape(2, 2)):
            ...
            tile_img: Image.Image = self.tileset_widget.main_gui.tiles\
                [tile['palette_idx']][tile['tile_idx']]
        ...
        item = QGraphicsPixmapItem(QPixmap.fromImage(
    ...
    def update_selection_box(self):
        ...
        self.tileset_widget.set_selection(select_blocks(
    ...
    def mouseMoveEvent(self, event: QGraphicsSceneMouseEvent):
        ...
        pos = event.scenePos()
        x, y = int(pos.x() * 10 / 8 / self.tileset_widget.zoom_slider.value()), \
        ...
        if 2 > x >= 0 and 2 > y >= 0:
            if self.last_draw is not None and self.last_draw != (x, y):
                ...
                selection = self.tileset_widget.selection[: min(2 - y,
                                                                selection_height),
                                                : min(2 - x, selection_width)].copy()
                block = self.tileset_widget.selected_block
                # Extract the old tiles
                tiles_old = block[int(self.layer), y : y + selection.shape[0],
                                  x : x + selection.shape[1]].copy()
                self.tileset_widget.undo_stack.push(history.SetTiles(
                    self.tileset_widget, self.tileset_widget.selected_block,
                    int(self.layer),x, y, selection.copy(), tiles_old.copy()
                ))
