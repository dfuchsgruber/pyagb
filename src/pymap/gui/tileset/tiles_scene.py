...
class TilesScene(QGraphicsScene, TilesetChildWidgetMixin):
    ...
    def ensure_selection_rect_visible(self):
        ...
        if self.selection_rect is not None:
            self.tileset_widget.tiles_scene_view.ensureVisible(self.selection_rect.rect())
    ...
    def select_tiles(self):
        ...
        tiles = tiles_pool[self.tileset_widget.tiles_palette_combobox.currentIndex(),
                           self.tileset_widget.tiles_flip]
        if self.selection_box is not None:
            self.tileset_widget.set_selection(render.select_blocks(
                tiles, *self.selection_box
            ))
    ...
    def update_selection_rect(self):
        ...
        if self.is_drawing:
            ...
            self.selection_rect.setRect(int(scale * x0), int(scale * y0),
                                        int(scale * (x1 - x0)), int(scale * (y1 - y0)))
        else:
